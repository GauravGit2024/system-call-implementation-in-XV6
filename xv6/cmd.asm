...

{
  ...
  1c:	7e 3d                	jle    5b <main+0x5b>
	...
	int n = argc;
    int pid = fork();
  1e:	e8 f8 02 00 00       	call   31b <fork>
    if(pid == 0){
  23:	85 c0                	test   %eax,%eax
  25:	75 48                	jne    6f <main+0x6f>
	char *args[n];
  27:	8d 47 ff             	lea    -0x1(%edi),%eax
  2a:	8d 0c bd 00 00 00 00 	lea    0x0(,%edi,4),%ecx
  31:	89 e7                	mov    %esp,%edi
    if(pid == 0){
  33:	89 e6                	mov    %esp,%esi
	char *args[n];
  35:	89 45 e4             	mov    %eax,-0x1c(%ebp)
  38:	8d 41 0f             	lea    0xf(%ecx),%eax
  3b:	89 c2                	mov    %eax,%edx
  3d:	25 00 f0 ff ff       	and    $0xfffff000,%eax
  42:	83 e2 f0             	and    $0xfffffff0,%edx
  45:	29 c7                	sub    %eax,%edi
  47:	39 fc                	cmp    %edi,%esp
  49:	74 2e                	je     79 <main+0x79>
  ...
  5b:	52                   	push   %edx
  5c:	ff 33                	push   (%ebx)
  5e:	68 f8 07 00 00       	push   $0x7f8
  63:	6a 01                	push   $0x1
  65:	e8 66 04 00 00       	call   4d0 <printf>
        exit();
  6a:	e8 b4 02 00 00       	call   323 <exit>
	}
	// printf(1, "args: %s\n", args[0]);
	args[n-1] = 0;
	exec(args[0], args);
	...
	wait();
  6f:	e8 b7 02 00 00       	call   32b <wait>
    }
	exit();
  74:	e8 aa 02 00 00       	call   323 <exit>
	char *args[n];
  79:	81 e2 ff 0f 00 00    	and    $0xfff,%edx
  7f:	29 d4                	sub    %edx,%esp
  81:	85 d2                	test   %edx,%edx
  83:	74 05                	je     8a <main+0x8a>
  85:	83 4c 14 fc 00       	orl    $0x0,-0x4(%esp,%edx,1)
  8a:	8d 7c 24 03          	lea    0x3(%esp),%edi
  8e:	89 f8                	mov    %edi,%eax
  90:	83 e7 fc             	and    $0xfffffffc,%edi
  93:	c1 e8 02             	shr    $0x2,%eax
  96:	89 fa                	mov    %edi,%edx
  98:	89 45 e0             	mov    %eax,-0x20(%ebp)
  9b:	8d 43 04             	lea    0x4(%ebx),%eax
  9e:	01 cb                	add    %ecx,%ebx
		args[i-1] = argv[i];
  a0:	8b 08                	mov    (%eax),%ecx
	for(int i=1; i<n; i++){
  a2:	83 c0 04             	add    $0x4,%eax
  a5:	83 c2 04             	add    $0x4,%edx
		args[i-1] = argv[i];
  a8:	89 4a fc             	mov    %ecx,-0x4(%edx)
	for(int i=1; i<n; i++){
  ab:	39 d8                	cmp    %ebx,%eax
  ad:	75 f1                	jne    a0 <main+0xa0>
	args[n-1] = 0;
  af:	8b 45 e4             	mov    -0x1c(%ebp),%eax
  b2:	c7 04 87 00 00 00 00 	movl   $0x0,(%edi,%eax,4)
	exec(args[0], args);
  b9:	50                   	push   %eax
  ba:	50                   	push   %eax
  bb:	57                   	push   %edi
  bc:	8b 45 e0             	mov    -0x20(%ebp),%eax
  bf:	ff 34 85 00 00 00 00 	push   0x0(,%eax,4)
  c6:	e8 90 02 00 00       	call   35b <exec>
  cb:	89 f4                	mov    %esi,%esp
  cd:	eb a5                	jmp    74 <main+0x74>
  cf:	90                   	nop
 ...
 42c:	89 d1                	mov    %edx,%ecx
{
 ...
 459:	0f b6 92 7c 08 00 00 	movzbl 0x87c(%edx),%edx
 ...
 4c1:	2e 8d b4 26 00 00 00 	lea    %cs:0x0(%esi,%eiz,1),%esi
 4c8:	00 
 ...
 4d9:	8b 5d 0c             	mov    0xc(%ebp),%ebx
{
 ...
 549:	ff 24 85 24 08 00 00 	jmp    *0x824(,%eax,4)
        putc(fd, c);
      }
      state = 0;
    }
 ...
 653:	2e 8d 74 26 00       	lea    %cs:0x0(%esi,%eiz,1),%esi
          s = "(null)";
 658:	ba 1c 08 00 00       	mov    $0x81c,%edx
        while(*s != 0){
 ...
 671:	a1 30 0b 00 00       	mov    0xb30,%eax
{
 ...
 6b1:	5b                   	pop    %ebx
  freep = p;
 6b2:	89 15 30 0b 00 00    	mov    %edx,0xb30
}
 6b8:	5e                   	pop    %esi
 6b9:	5f                   	pop    %edi
 6ba:	5d                   	pop    %ebp
 ...
 6e9:	03 43 fc             	add    -0x4(%ebx),%eax
  freep = p;
 6ec:	89 15 30 0b 00 00    	mov    %edx,0xb30
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
 ...
 709:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 70c:	8b 3d 30 0b 00 00    	mov    0xb30,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
 ...
 74f:	73 4f                	jae    7a0 <malloc+0xa0>
    ...
    if(p == freep)
 751:	8b 3d 30 0b 00 00    	mov    0xb30,%edi
 ...
 77a:	e8 f1 fe ff ff       	call   670 <free>
  return freep;
 77f:	8b 15 30 0b 00 00    	mov    0xb30,%edx
 ...
 7ac:	89 70 04             	mov    %esi,0x4(%eax)
      freep = prevp;
 7af:	89 15 30 0b 00 00    	mov    %edx,0xb30
}
 ...
 7c7:	0b 00 00 
    base.s.size = 0;
 7ca:	bf 34 0b 00 00       	mov    $0xb34,%edi
    base.s.ptr = freep = prevp = &base;
 7cf:	c7 05 34 0b 00 00 34 	movl   $0xb34,0xb34
 7d6:	0b 00 00 
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 7d9:	89 fa                	mov    %edi,%edx
    base.s.size = 0;
 7db:	c7 05 38 0b 00 00 00 	movl   $0x0,0xb38
 7e2:	00 00 00 
    if(p->s.size >= nunits){
